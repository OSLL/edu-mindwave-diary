...
main( void ) {
    ...
    int   dllVersion   = 0; // версия используемой библиотеки
    int   connectionId = 0; // номер текущего соединения
    int   packetsRead  = 0;
    int   errCode      = 0; // номер возникшей ошибки
    ...
    while( difftime(time(NULL), startTime) < secondsToRun ) {
        ...
        do {
            ...
            packetsRead = TG_ReadPackets( connectionId, 4 );
